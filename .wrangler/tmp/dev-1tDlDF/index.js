...
// .wrangler/tmp/bundle-sbWwRz/checked-fetch.js
...
var escapeRe = /[&<>'"]/;
var stringBufferToString = async (buffer2, callbacks) => {
  let str = "";
  callbacks ||= [];
  const resolvedBuffer = await Promise.all(buffer2);
...
var escapeToBuffer = (str, buffer2) => {
  ...
  for (index = match; index < str.length; index++) {
    ...
    buffer2[0] += str.substring(lastIndex, index) + escape2;
    lastIndex = index + 1;
  }
  buffer2[0] += str.substring(lastIndex, index);
};
var resolveCallbackSync = (str) => {
  ...
  const buffer2 = [str];
  const context = {};
  callbacks.forEach((c) => c({ phase: HtmlEscapedCallbackPhase.Stringify, buffer: buffer2, context }));
  return buffer2[0];
};
...
var LocationSchema = z.string().nullable();
var TimestampSchema = z.coerce.date();
var NullableTimestampSchema = TimestampSchema.nullable();
var NullableStringSchema = z.string().nullable();
...
var PaginationSchema2 = z.object({
...
var QuerySchema = PaginationSchema2.merge(SortSchema);
var DateRangeSchema = z.object({
  startDate: z.string().datetime().optional(),
  endDate: z.string().datetime().optional()
});
var EventQuerySchema = QuerySchema.merge(DateRangeSchema);

// src/services/events.ts
async function fetchEvents(sql, params) {
  ...
  const sortColumn = validSortColumns.includes(params.sortBy) ? params.sortBy : "time";
  ...
  const baseQuery = sql`
    ...
    LEFT JOIN "SensorData" sd ON e.id = sd."eventId"
    LEFT JOIN "Sensors" s ON sd."sensorId" = s.id
    LEFT JOIN "SensorTypes" st ON s."typeId" = st.id
    ...
    OFFSET ${offset}
  `;
  console.log("SQL generated, executing queries...");
  ...
  `;
  try {
    const [countResult, events2] = await Promise.all([countQuery, baseQuery]);
    console.log(
...
async function fetchEventById(sql, id) {
  const query = sql`
    SELECT 
      e.id,
      e.time,
...
var events = new OpenAPIHono();
function formatResponse(events2, totalCount, params) {
...
var getEventsRoute = createRoute({
  method: "get",
  path: "/events",
  tags: ["Events"],
...
events.openapi(getEventRoute, async (c) => {
  ...
  const sql = src_default(c.env.HYPERDRIVE.connectionString);
  try {
...
events.openapi(getEventsRoute, async (c) => {
  ...
  const sql = src_default(c.env.HYPERDRIVE.connectionString);
  try {
    const { events: events2, totalCount } = await fetchEvents(sql, params);
    return c.json(formatResponse(events2, totalCount, params));
  } catch (error) {
    return c.json(
      {
        error: "Internal server error",
...
var getDeviceEventsRoute = createRoute({
  method: "get",
...
var html = (strings, ...values2) => {
  ...
  for (let i = 0, len = strings.length - 1; i < len; i++) {
    buffer2[0] += strings[i];
    const children = Array.isArray(values2[i]) ? values2[i].flat(Infinity) : [values2[i]];
  ...
  return buffer2.length === 1 ? "callbacks" in buffer2 ? raw(resolveCallbackSync(raw(buffer2[0], buffer2.callbacks))) : raw(buffer2[0]) : stringBufferToString(buffer2, buffer2.callbacks);
};
...
// .wrangler/tmp/bundle-sbWwRz/middleware-insertion-facade.js
var envWrappers = [void 0].filter(Boolean);
var facade = {
  ...src_default2,
  envWrappers,
  middleware: [
    middleware_ensure_req_body_drained_default,
    ...src_default2.middleware ? src_default2.middleware : []
  ].filter(Boolean)
};
...
// .wrangler/tmp/bundle-sbWwRz/middleware-loader.entry.ts
