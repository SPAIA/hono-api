...
// .wrangler/tmp/bundle-gWZxLX/checked-fetch.js
...
var QuerySchema = PaginationSchema.merge(SortSchema);
...
var ParamsSchema = z.object({
  deviceUuid: z.string().uuid("Must be a valid UUID")
});
  ...
  try {
    ...
    const query = sql`
      SELECT 
        ...
        FROM "Events" e
        LEFT JOIN "Regions" r ON e.id = r."eventId"
        GROUP BY e.id
    ...
    const [countResult, events2] = await Promise.all([
...
events.get("/:deviceUuid", zValidator("param", ParamsSchema), async (c) => {
  const { deviceUuid } = c.req.valid("param");
  try {
    const sql = src_default(c.env.HYPERDRIVE.connectionString);
...
app.route("/events", events_default);
...
// .wrangler/tmp/bundle-gWZxLX/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gWZxLX/middleware-loader.entry.ts
